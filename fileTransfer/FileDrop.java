...
/**
<<<<<<< HEAD
 * This class makes it easy to drag and drop files from the operating system to a Java program. Any <tt>java.awt.Component</tt> can be dropped onto, but only <tt>javax.swing.JComponent</tt>s will indicate the drop event with a changed border.
 * <p/>
...
public class FileDrop {
	...
	/**
	 * Constructs a {@link FileDrop} with a default light-blue border and, if <var>c</var> is a {@link java.awt.Container}, recursively sets all elements contained within as drop targets, though only the top level container will change borders.
	 * 
	 * @param c
	...
	/**
	 * Constructor with a default border and the option to recursively set drop targets. If your component is a <tt>java.awt.Container</tt>, then each of its children components will also listen for drops, though only the parent will change borders.
	 * 
	 * @param c
	 *            Component on which files will be dropped.
	 * @param recursive
	...
	/**
	 * Constructor with a default border and debugging optionally turned on. With Debugging turned on, more status messages will be displayed to <tt>out</tt>. A common way to use this constructor is with <tt>System.out</tt> or <tt>System.err</tt>. A <tt>null</tt> value for the parameter <tt>out</tt>
	...
	/**
	 * Constructor with a default border, debugging optionally turned on and the option to recursively set drop targets. If your component is a <tt>java.awt.Container</tt>, then each of its children components will also listen for drops, though only the parent will change borders. With Debugging
	 * turned on, more status messages will be displayed to <tt>out</tt>. A common way to use this constructor is with <tt>System.out</tt> or <tt>System.err</tt>. A <tt>null</tt> value for the parameter <tt>out</tt> will result in no debugging output.
	 * 
	 * @param out
	 *            PrintStream to record debugging info or null for no debugging.
	 * @param out
	...
	/**
	 * Constructor with a specified border and the option to recursively set drop targets. If your component is a <tt>java.awt.Container</tt>, then each of its children components will also listen for drops, though only the parent will change borders.
	 * 
	 * @param c
	 *            Component on which files will be dropped.
	 * @param dragBorder
	...
	/**
	 * Constructor with a specified border and debugging optionally turned on. With Debugging turned on, more status messages will be displayed to <tt>out</tt>. A common way to use this constructor is with <tt>System.out</tt> or <tt>System.err</tt>. A <tt>null</tt> value for the parameter
	...
	/**
	 * Full constructor with a specified border and debugging optionally turned on. With Debugging turned on, more status messages will be displayed to <tt>out</tt>. A common way to use this constructor is with <tt>System.out</tt> or <tt>System.err</tt>. A <tt>null</tt> value for the parameter
	 * <tt>out</tt> will result in no debugging output.
	 * 
	 * @param out
	 *            PrintStream to record debugging info or null for no debugging.
	...
	/**
	 * Removes the drag-and-drop hooks from the component and optionally from the all children. You should call this if you add and remove components after you've set up the drag-and-drop. This will recursively unregister all components contained within <var>c</var> if <var>c</var> is a
	...
	/**
	 * At last an easy way to encapsulate your custom objects for dragging and dropping in your Java programs! When you need to create a {@link java.awt.datatransfer.Transferable} object, use this class to wrap your object. For example:
	 * 
	 * <pre>
	 * <code>
	 *      ...
	 ...
	 * </pre>
	 * 
	 * The {@link java.awt.datatransfer.DataFlavor} associated with {@link TransferableObject} has the representation class <tt>net.iharder.dnd.TransferableObject.class</tt> and MIME type <tt>application/x-net.iharder.dnd.TransferableObject</tt>. This data flavor is accessible via the static
	...
	public static class TransferableObject implements java.awt.datatransfer.Transferable {
		...
		/**
		 * Creates a new {@link TransferableObject} that wraps <var>data</var>. Along with the {@link #DATA_FLAVOR} associated with this class, this creates a custom data flavor with a representation class determined from <code>data.getClass()</code> and the MIME type
		...
		/**
		 * Creates a new {@link TransferableObject} that will return the object that is returned by <var>fetcher</var>. Along with the {@link #DATA_FLAVOR} associated with this class, this creates a custom data flavor with a representation class <var>dataClass</var> and the MIME type
		 * <tt>application/x-net.iharder.dnd.TransferableObject</tt>.
		 * 
		 * @see Fetcher
		 * @param dataClass
		...
		/**
		 * Returns the data encapsulated in this {@link TransferableObject}. If the {@link Fetcher} constructor was used, then this is when the {@link Fetcher#getObject getObject()} method will be called. If the requested data flavor is not supported, then the {@link Fetcher#getObject getObject()}
		...
		/**
		 * Instead of passing your data directly to the {@link TransferableObject} constructor, you may want to know exactly when your data was received in case you need to remove it from its source (or do anyting else to it). When the {@link #getTransferData getTransferData(...)} method is called
